...
def doSiur(data, amountOfSoldiers, sevev):
...
def KafKafA(data, amountOfKKA, sevev):
    divisions = seperate_to_divisions(data)
    div1, div2, div3, div4 = [], [], [], []
    if sevev == "מפ":
        highest1 = highest(divisions[0], "Kaf Kaf A:")
...
def cycle(data, amountOfSoldiers, amountOfSiurim, amountOfKKA, debug, num, sevev):
    soldiers = []
    ...
    if num == 1:
        ...
        kafkafa, data = KafKafA(data, amountOfKKA, sevev)
        for i in kafkafa:
            for j in data:
                if not debug:
                    if i["Name:"] == j["Name:"]:
    ...
    if num == 1 or (num == 3 and amountOfSiurim > 1) or (num == 5 and amountOfSiurim > 2):
        siur, data = doSiur(data, amountOfSoldiers, sevev)
        for i in siur:
            if not debug:
                i["Siur:"] += 1
                i["Resting Hours:"] = 16
...
def json_to_iter(sevev, inactive):
    with open("soldiers.json", "r") as f:
        data = json.load(f)
        for i in data:
            for n in i:
...
def computeList(amountOfSoldiers, amountOfSiurim, amountOfKKA, attempts, sevev, debug):
    ...
    for j in range(attempts):
        ...
        soldiers = []
        for i in range(1, 7):
            temp, data_to_iter = (cycle(data_to_iter, amountOfSoldiers, amountOfSiurim, amountOfKKA, debug, i, sevev))
            for key in temp:
